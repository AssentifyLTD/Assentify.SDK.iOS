...
class ViewController: UIViewController , AssentifySdkDelegate,FaceMatchDelegate,SubmitDataDelegate{
...
//
    ...
    func onSend() {
        ...
        assentifySdk?.startSubmitData(submitDataDelegate: self, submitRequestModel: [SubmitRequestModel(stepId: 1, stepDefinition: "s1", extractedInformation: ["here" : "here1","here2" : "here1"]),SubmitRequestModel(stepId: 2, stepDefinition: "s2", extractedInformation: ["here" : "here1"])])
        let currentDate = Date()
        let formatter = DateFormatter()
