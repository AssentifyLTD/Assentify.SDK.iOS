...
public struct SecurePhoneInput: View {
    ...
    @ViewBuilder
    private func codeDropdownList() -> some View {
        ScrollView {
            LazyVStack(spacing: 0) {
                ForEach(options, id: \.self) { option in
                    Button {
                        ...
                        HStack(spacing: 10) {
                            ...
                            Text(option.dialCode)
                                .foregroundColor(Color(BaseTheme.baseTextColor))
                            Spacer()
                            if option.code2.caseInsensitiveCompare(selectedIso2) == .orderedSame {
                    ...
                    Divider()
                        .background(Color(BaseTheme.baseTextColor).opacity(0.1))
                }
            }
        }
